  ...
  try {
    ...
    for (const report of reports) {
        ...
        const waterConsumptionBySTPCapacityArray = [
          {
            id,
            stpCapacity: "65KLD",
            inletReadingClosing: kld65.inletReadingClosing,
            inletReadingOpening: kld65.inletReadingOpening,
            outletReadingClosing: kld65.outletReadingClosing,
            outletReadingOpening: kld65.outletReadingOpening,
            waterHardnessPPM: kld65.waterHardnessPPM,
            waterTDSPPM: kld65.waterTDSPPM,
            ph: kld65.ph,
            date: date.slice(0, -1),
          },
          {
            id,
            stpCapacity: "90KLD",
            inletReadingClosing: kld90.inletReadingClosing,
            inletReadingOpening: kld90.inletReadingOpening,
            outletReadingClosing: kld90.outletReadingClosing,
            outletReadingOpening: kld90.outletReadingOpening,
            waterHardnessPPM: kld90.waterHardnessPPM,
            waterTDSPPM: kld90.waterTDSPPM,
            ph: kld90.ph,
            date: date.slice(0, -1),
          },
        ];

        const stpReportQuery = `INSERT INTO stpReport(id, stpCapacity, inletReadingClosing,
      ...
      //   // //CCTV  Status
        ...
        const cctvQuery = `INSERT INTO cctv (id,cctv,cctvStatus,date)
            VALUES(?,?,?,?)`;
        ...
        const groundWaterTankArray = [
          {
            ...
            statusPercentage: statusPercentage1,
            date: date.slice(0, -1),
          },
          {
            id,
            tankName: "fireWaterTank-1",
            statusPercentage: statusPercentage2,
            date: date.slice(0, -1),
          },
          {
            id,
            tankName: "fireWaterTank-2",
            statusPercentage: statusPercentage3,
            date: date.slice(0, -1),
          },
        ];
        const groundWaterTankQuery = `INSERT INTO groundWaterReport (id,tankName,statusPercentage,date)
            VALUES(?,?,?,?)`;
      ...
      //   // //Critical Equipment
        ...
        const { transformer, ups, dg } = jsondata;
        ...
        const criticalEquipmentQuery = `INSERT INTO criticalEquipmentReport(id,criticalEquipment,workingStatus,date)
        VALUES(?,?,?,?)`;
      ...
      //   // HSD STOCK
        ...
        const hsdStockQuery = `INSERT INTO hsdStock (
          id,
      ...
      //   // //dg status
        ...
        async function fetchPreviousDayDgStatus(currentDate) {
          const previousDayDgStatusQuery = `
            ...
            LIMIT 1
          `;
          const [previousDayDgStatusRows] = await dbConnection.execute(
            previousDayDgStatusQuery,
            [currentDate]
          );
          return (
            previousDayDgStatusRows[0] || {
              presentDG250KVA: 0,
        ...
        // // Fetch the previous day's DG status
      ...
      //   // //DG B Check
        ...
        const dgBQuery = `INSERT INTO dgBCheck (id,dgRunningHrs250KVA,fireEngineRunningHrs125KVA,dgNextBCheck250KVA,fireEngineNextBCheck125KVA,date)
        VALUES(?,?,?,?,?,?) `;
      ...
      //   //Breakdown Status
        ...
        if (breakdownStatus) {
          ...
          const breakdownQuery = `INSERT INTO breakdownStatusReport (id,breakdownStatus, projectName, projectDetails, breakdownOnDate, dateOfEscalation,date)
          VALUES (?,?, ?, ?, ?, ?,?)`;
      ...
      //   //Activity Status
        ...
        const activityQuery = `INSERT INTO activityReport (id, activityDetails, activityDate)
         VALUES(?,?,?)`;
      ...
      //   // //UPS STATUS
        ...
        const upsQuery = `INSERT INTO upsReport(id,rating,maxLoad,batteryVoltage,serviceDate,date)
            VALUES(?,?,?,?,?,?) `;
