...
export const lambdaHandler = async (event, context) => {
  try {
    ...
    for (const report of reports) {
      ...
      await Promise.all(
        usageByMeters.map(async (meterData) => {
          ...
          const electricityUsageQuery = `
            INSERT INTO electricityUsage (id, meterName, presentKvah, presentKwh, previousKvah, previousKwh, date,pf)
            VALUES (?, ?, ?, ?, ?, ?, ?,?)
          `;
      ...
      const criticalEquipmentQuery = `INSERT INTO criticalEquipmentReport (id, criticalEquipment, workingStatus, date, remarks) VALUES (?,?,?,?,?)`;
